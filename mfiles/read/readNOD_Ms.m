function [b, b2] = readNOD_Ms(varargin)
    ...
    % tmp = getNextLine(fn,'criterion','with','keyword',...
    %     '## NODEWISE');
    % tmp = regexprep(tmp,'##\s*NODEWISE\s*RESULTS',''); % Yuxuan
    tmp           = textscan(tmp,'%f ');
    ...
    for n=1:output_no
        ...
        if tmp  ~= -1
            ...
            [ b(n).itout,b(n).durn,b(n).tout] = deal(tmp{:});
            ...
            b(n).label = getOutputLabelName( tmp );
            fmt        = repmat('%f ',1, length(b(n).label));
            b(n).terms = textscan(fn,fmt); % ,o2.nn
        else
